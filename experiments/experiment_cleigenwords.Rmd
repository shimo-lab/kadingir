...
load("./../res_cleigenwords_es-en.Rdata")
load("./../res_cllsa.Rdata")
```
...
```{r}
translate.table <- read.csv("translate_table_es-en_es_reduced.csv", sep = ",", as.is = TRUE)[1:3000, ]
head(translate.table[ , c("es","en")])
```
...
for (i.query in seq(queries)) {
  ...
  answers.cllsa.query <- names(MostSimilar(res.cllsa$representations.word, row.names(res.cllsa$representations.word), positive = c(query), distance = "cosine", language.search = "en", topn = topn))
...
```{r, results = "asis"}
table.accuracy <- matrix(accuracy, nrow = 3, dimnames = list(c("Edit distance", "CL-LSA", "CL-Eigenwords"), c("@1", paste0("@", topn))))
kable(table.accuracy)
```