...
```{r, results='hide'}
if (!file.exists("run_cleigenwords.html")) {
...
V2 <- V
row.names(V2) <- vocab.words
pr.v2 <- prcomp(V2[queries,])
...
```{r}
path.cllsa.rdata <- "res_cllsa_0125.Rdata"
...
# 頻出上位 $n$ 件の単語をクエリとして使ったときの Precision\@5 のグラフ
```{r}
