...
source("./../mceigendocs.R", chdir=TRUE)
```

```{r}
paths.corpus <- c("./../data/corpus/movie_imdb/imdb_reviews.txt")
res.eigendocs <- MCEigendocs(paths.corpus, max.vocabulary=10000, dim.internal=50,
                             window.sizes = c(4), aliases=c("en"), plot = TRUE)
save(res.eigendocs, file = "./../res_eigendocs_imdb.Rdata")
```

## Check vector representations of words
```{r}
p_cumsum <- res.eigendocs$svd$p_cumsum
word_vector <- res.eigendocs$svd$V[p_cumsum[1]:p_cumsum[2], ]
vocab_words <- unlist(res.eigendocs$vocab.words)

MostSimilar(word_vector, vocab_words,
...
## wordsim353
```{r}
TestWordsim353(word_vector, vocab_words, path = "./../test/combined.csv")
```


## Sentiment analysis using IMDb dataset 1 : unregularized
```{r}
vectors <- res.eigendocs$svd$V[(p_cumsum[3]+1):p_cumsum[4], ]
csv <- read.csv("./../data/corpus/movie_imdb/imdb_id_rate.csv")
